...
export class CreateSchemas1771938525932 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const extractableRecordsExists = await queryRunner.hasTable('extractable_records');
    if (!extractableRecordsExists) {
    ...
    // --- Audit Log Table ---
    const auditLogExists = await queryRunner.hasTable('audit_log');
    if (!auditLogExists) {
      await queryRunner.query(`
