...
```sh
docker build -t 3d-extractable-management-migrations:latest -f migrations.Dockerfile .
```
Run image:
```sh
docker run -it --rm --network host 3d-extractable-management-migrations:latest
```
...
Via env variables:
```sh
docker run -it -e DB_USERNAME=VALUE  -e DB_PASSWORD=VALUE -e DB_NAME=VALUE -e DB_TYPE=VALUE -e DB_HOST=VALUE -e DB_PORT=VALUE --rm --network host 3d-extractable-management-migrations:latest
```
...
```
```sh
docker run -it --rm -e NODE_ENV=production --network host -v /path/to/proudction.json:/usr/app/config/production.json 3d-extractable-management-migrations:latest
```
---
